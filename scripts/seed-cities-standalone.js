...
const citiesData = {
  ...
  'Srivaikuntam', 'Srivilliputhur Rural', 'Srirangam Rural', 'Srirangam Town', 'Srirangapatnam',
  'Sunguvarchatram Rural', 'Surandai', 'T. Kallupatti', 'Tenkasi Rural',
  'Thammampatti Town', 'Thanjavur Rural', 'Tharamangalam', 'Tharangambadi',
  'Theni Rural', 'Thenkarai', 'Thirumangalam Rural', 'Thirumayam Rural',
  'Thirunageswaram', 'Thirunindravur', 'Thiruparappu', 'Thirupathur',
  'Thirupuvanam Rural', 'Thiruthani Rural', 'Thiruthuraipoondi Rural',
  'Thiruvarur Rural', 'Thoothukudi Rural', 'Thottiyam', 'Thuraiyur Rural',
  'Tindivanam Rural', 'Tiruchengode', 'Tiruchengode Rural', 'Tirukkoyilur',
  'Tirunelveli Rural', 'Tirupathur', 'Tiruppur Rural', 'Tiruvallur Rural',
  'Tiruvannamalai Rural', 'Tiruvethipuram', 'Tittakudi', 'Udagamandalam Rural',
  'Udumalaipettai', 'Uthangarai Town', 'Uthukottai Rural', 'Vadakkuvalliyur',
  'Vadipatti', 'Valangaiman Rural', 'Vallam', 'Valliyur', 'Vandavasi Rural',
  'Vaniyambadi Rural', 'Vedaranyam Rural', 'Vellakoil Rural', 'Vellore Fort Area Rural',
  'Vellore Rural', 'Velur Rural', 'Vilathikulam', 'Viluppuram Rural',
  'Villupuram Rural', 'Virudhunagar Rural', 'Watrap Rural'
  ],
...
async function seedCities() {
  ...
  try {
    ...
    // Get the highest existing city serial number
    const lastCity = await citiesCollection.findOne({}, { sort: { serialNo: -1 } });
    let citySerialNo = lastCity ? lastCity.serialNo + 1 : 1;
    ...
    for (const state of states) {
      ...
      if (stateCities) {
        console.log(`Adding cities for ${state.name} (${stateCode})`);
        ...
        for (const cityName of stateCities) {
          ...
          if (!existingCity) {
            const cityDoc = {
              ...
              countryName: state.countryName || 'India',
            ...
            console.log(`Skipped: ${cityName} (already exists)`);
          }
        }
      } else {
        console.log(`No cities data found for state: ${state.name} (${stateCode})`);
      }
    }
