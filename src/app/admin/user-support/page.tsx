...
export default function UserSupportPage() {
  ...
  const handleReplyTicket = async () => {
    if (!selectedTicketId || !replyText.trim()) {
      toast.error('Please select a ticket and enter a reply message.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to add reply: ${responseData.error || 'Unknown error'}`);
      }
    } catch (error) {
      toast.error('Error adding reply: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleMarkResolved = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to mark ticket as resolved: ${responseData.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleCloseTicket = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to close ticket: ${responseData.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to delete ticket: ${responseData.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const debugToken = () => {
    ...
    if (!token || token.trim().length === 0) {
      toast.error('Invalid or missing admin token. Please login again.');
      return false;
    }
    return true;
  };
  ...
  const fetchSupportTickets = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to fetch tickets: ${errorData.error || 'Unknown error'}`);
      }
    } catch (error) {
