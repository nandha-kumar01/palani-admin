...
export default function NotificationsPage() {
      ...
      <AdminLayout>
        <Box sx={{ p: 3, textAlign: 'center' }}>
          <Box sx={{ 
            ...
            <ErrorIcon sx={{ fontSize: 64, color: '#856404', mb: 2 }} />
            <Typography variant="h5" sx={{ color: '#856404', mb: 2, fontWeight: 600 }}>
              Unable to Load Notifications
            </Typography>
            <Typography variant="body1" sx={{ color: '#856404', mb: 3 }}>
              We're having trouble connecting to the notification service. This could be due to:
            </Typography>
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      if (!token) {
        ...
        toast.error('Please login to access notifications', { containerId: 'notifications-toast' });
        return;
      }
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          toast.error('Session expired. Please login again.', { containerId: 'notifications-toast' });
      ...
      if (result.success) {
        ...
        setError(errorMsg);
        toast.error(errorMsg, { containerId: 'notifications-toast' });
      }
    } catch (error: any) {
      console.error('Fetch notifications error:', error);
      const errorMessage = error.message || 'Network error occurred';
      setError(`Failed to load notifications: ${errorMessage}`);
      toast.error(`Failed to fetch notifications: ${errorMessage}`, { containerId: 'notifications-toast' });
  ...
  const handleCreateNotification = async () => {
    try {
      ...
      if (!formData.title.trim()) {
        toast.error('Please enter a notification title', { containerId: 'notifications-toast' });
        return;
      }
      
      if (!formData.message.trim()) {
        toast.error('Please enter a notification message', { containerId: 'notifications-toast' });
        return;
      }
      ...
      if (!token) {
        toast.error('No authentication token found. Please login again.', { containerId: 'notifications-toast' });
        return;
      }
      ...
      if (result.success) {
        toast.success('Notification created successfully!', { containerId: 'notifications-toast' });
        setCreateDialogOpen(false);
        resetForm();
        fetchNotifications();
      } else {
        toast.error(result.error || 'Failed to create notification', { containerId: 'notifications-toast' });
      }
    } catch (error: any) {
      console.error('Create notification error:', error);
      toast.error(`Failed to create notification: ${error.message || 'Network error'}`, { containerId: 'notifications-toast' });
    }
  };
  ...
  const handleSendNotification = async (notificationId: string) => {
    try {
      ...
      if (result.success) {
        toast.success('Notification sent successfully!', { containerId: 'notifications-toast' });
        fetchNotifications();
      } else {
        toast.error(result.error || 'Failed to send notification', { containerId: 'notifications-toast' });
      }
    } catch (error: any) {
      console.error('Send notification error:', error);
      toast.error(`Failed to send notification: ${error.message || 'Network error'}`, { containerId: 'notifications-toast' });
    }
  };
  ...
  const handleDeleteNotification = async (notificationId: string) => {
    try {
      ...
      if (result.success) {
        toast.success('Notification deleted successfully!', { containerId: 'notifications-toast' });
        fetchNotifications();
      } else {
        toast.error(result.error || 'Failed to delete notification', { containerId: 'notifications-toast' });
      }
    } catch (error: any) {
      console.error('Delete notification error:', error);
      toast.error(`Failed to delete notification: ${error.message || 'Network error'}`, { containerId: 'notifications-toast' });
    }
  };
    ...
    <AdminLayout>
      <Box sx={{ p: 3 }}>
        ...
        <ToastContainer
          ...
          containerId="notifications-toast"
        />
