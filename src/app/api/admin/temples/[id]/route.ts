...
async function updateTemple(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const temple = await withTimeout(await Temple.findByIdAndUpdate(
      params.id,
      updates,
...
async function deleteTemple(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const temple = await withTimeout(Temple.findByIdAndDelete(params.id), 15000, 'Database operation timeout');
